...
**scPaX** is an R package designed to compute and visualize the probability of accessibility (Paₓ) of regulatory regions from single‑cell ATAC‑seq (scATAC‑seq) datasets. It uses a balls‑into‑bins model to fit a nonlinear curve and estimate the key parameter *a*, then produces a summary table and two publication‑ready plots:
  * A **scatter plot** of Paₓ vs. number of fragments per region
  * A **histogram of** log₁₀(Paₓ) distribution
    

## Installation

Install development tools (if not already installed):
``` {r pre-instalation, eval = FALSE}
...
``` {r installation from directory, eval = FALSE}
# From within the scPaX project root
devtools::install()
```

## Usage
```{r scPaX example, eval = FALSE}
library(scPaX)
library(Seurat)
library(Signac)
...
## Saving Outputs
After running the function, you can save the output table as a .csv, .tsv, etc, and the plots as .png, .jpg, .pdf, etc.
``` {r scPaX saving, eval = FALSE}
...
#### Arguments
* *seurat_obj*: A Seurat object with a single‑cell ATAC assay.
...
* *scatter_plot*: ggplot2 object of Paₓ vs. fragments
* *histogram*: ggplot2 object of log₁₀(Paₓ) distribution

## Contact
If you have any comments or suggestions about scPaX please raise an issue or contact us:
